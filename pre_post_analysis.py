...
def create_charts_wa_with_gap_and_line():
    ...
    wa_data = wa_data[
        (wa_data["opioid_YEAR"] >= 2009) & (wa_data["opioid_YEAR"] <= 2015)
    ]
    ...
    aggregated_data["morphine_per_capita"] = (
        aggregated_data["total_morphine"] / aggregated_data["total_population"]
    )
    aggregated_data["deaths_per_capita"] = (
        aggregated_data["total_deaths"] / aggregated_data["total_population"]
    )
    ...
    deaths_model = smf.ols(
        "deaths_per_capita ~ relative_year", data=aggregated_data
    ).fit()
    ...
    fig.suptitle(
        "In 2012, WA Implemented Regulation for Opioids.\nThe figure shows the trend.",
        fontsize=20,
        weight="bold",
    ...
    axes[0].axvline(
        x=0, color=duke_colors["gray"], linestyle="--", linewidth=1.5, label="Year 2012"
    )
    axes[0].set_title(
        "Washington: Morphine g per Capita",
        fontsize=16,
        weight="bold",
        color=duke_colors["blue"],
    )
    ...
    axes[1].set_title(
        "Washington: Overdose Deaths per Capita",
        fontsize=16,
        weight="bold",
        color=duke_colors["blue"],
    )
    ...
    axes[1].set_ylabel(
        "Overdose Deaths per Capita", fontsize=14, color=duke_colors["gray"]
    )
...
def create_charts_fl_with_gap_and_line():
    final_dataset = pd.read_csv("final_dataset.csv")
    # Filter data for fl only
    fl_data = final_dataset[final_dataset["State"] == "FL"]

    # Ensure 'opioid_YEAR' is numeric and drop invalid rows
    fl_data["opioid_YEAR"] = pd.to_numeric(fl_data["opioid_YEAR"], errors="coerce")
    fl_data = fl_data.dropna(subset=["opioid_YEAR"])

    # Filter for years between 2006 and 2015
    fl_data = fl_data[
        (fl_data["opioid_YEAR"] >= 2007) & (fl_data["opioid_YEAR"] <= 2013)
    ]

    # Adjust the x-axis to use 2012 as the 0 point
    fl_data["relative_year"] = fl_data["opioid_YEAR"] - 2010

    # Group by relative year and aggregate the required metrics
    aggregated_data = (
        fl_data.groupby("relative_year")
        .agg(
            total_population=("pop_Population", "sum"),
            total_morphine=("opioid_morphine_equivalent_g", "sum"),
            total_deaths=("mort_overdose_deaths", "sum"),
    ...
    fig, axes = plt.subplots(1, 2, figsize=(20, 7), facecolor=duke_colors["background"])
    fig.suptitle(
        "In 2010, FL Implemented Regulation for Opioids.\nThe figure shows the trend.",
        fontsize=20,
        weight="bold",
        color=duke_colors["blue"],
    )
    ...
    axes[0].axvline(
        x=0, color=duke_colors["gray"], linestyle="--", linewidth=1.5, label="Year 2010"
    )
    axes[0].set_title(
        "Florida: Morphine g per Capita",
        fontsize=16,
        weight="bold",
        color=duke_colors["blue"],
    )
    axes[0].set_xlabel("Years Relative to 2010", fontsize=14, color=duke_colors["gray"])
    axes[0].set_ylabel("Morphine g per Capita", fontsize=14, color=duke_colors["gray"])
    axes[0].grid(color=duke_colors["gray"], linestyle="--", linewidth=0.5, alpha=0.7)
    axes[0].legend(
        fontsize=12, loc="upper left", frameon=True, edgecolor=duke_colors["gray"]
    ...
    axes[1].set_title(
        "Florida: Overdose Deaths per Capita",
        fontsize=16,
        weight="bold",
        color=duke_colors["blue"],
    )
    axes[1].set_xlabel("Years Relative to 2010", fontsize=14, color=duke_colors["gray"])
    axes[1].set_ylabel(
        "Overdose Deaths per Capita", fontsize=14, color=duke_colors["gray"]
    )
    axes[1].grid(color=duke_colors["gray"], linestyle="--", linewidth=0.5, alpha=0.7)
