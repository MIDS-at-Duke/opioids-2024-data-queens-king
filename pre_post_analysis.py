...
def create_charts_fl_with_gap_and_line():
    ...
    # Filter for years between 2006 and 2015
    fl_data = fl_data[(fl_data['opioid_YEAR'] >= 2007) & (fl_data['opioid_YEAR'] <= 2013)]
    
    # Adjust the x-axis to use 2012 as the 0 point
    fl_data['relative_year'] = fl_data['opioid_YEAR'] - 2010
    
    # Group by relative year and aggregate the required metrics
    aggregated_data = fl_data.groupby('relative_year').agg(
        total_population=('pop_Population', 'sum'),
        total_morphine=('opioid_morphine_equivalent_g', 'sum'),
        total_deaths=('mort_overdose_deaths', 'sum')
    ).reset_index()
    ...
    # Create side-by-side subplots
    fig, axes = plt.subplots(1, 2, figsize=(20, 7), facecolor=duke_colors['background'])
    fig.suptitle('In 2010, FL Implemented Regulation for Opioids.\nThe figure shows the trend.', 
                 fontsize=20, weight='bold', color=duke_colors['blue'])
    ...
    axes[0].axvline(x=0, color=duke_colors['gray'], linestyle='--', linewidth=1.5, label='Year 2010')
    axes[0].set_title('Florida: Morphine g per Capita', fontsize=16, weight='bold', color=duke_colors['blue'])
    ...
    axes[1].axvline(x=0, color=duke_colors['gray'], linestyle='--', linewidth=1.5, label='Year 2010')
    axes[1].set_title('Florida: Overdose Deaths per Capita', fontsize=16, weight='bold', color=duke_colors['blue'])
