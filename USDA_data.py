...
def get_unemployment_data(filepath="USDA_data/Unemployment.csv"):
    ...
    unemployment_all = unemployment_all[
        unemployment_all["State"].isin(["FL", "WA", "OR", "GA", "OK", "AL", "CO", "ME"])
    ]
    ...
    unemployment_all_data = unemployment_all_pivot[
        [
            "FIPS_Code",
            "State",
            "Area_Name",
...
def get_income_data(filepath="USDA_data/Unemployment.csv"):
    ...
    pre_income = pre_income[
        pre_income["State"].isin(["FL", "WA", "OR", "GA", "OK", "AL", "CO", "ME"])
    ]
    med_income_data = pre_income[
        [
            "FIPS_Code",
...
def get_USDA_data():
    poverty_data = get_poverty_data(filepath="./data/USDA_data/Poverty.csv")
    population2010_data = get_population_data(
        filepath="./data/USDA_data/Population.csv"
    )
    unemployment_all_data = get_unemployment_data(
        filepath="./data/USDA_data/Unemployment.csv"
    )
    med_income_data = get_income_data(filepath="./data/USDA_data/Unemployment.csv")
    ...
    print(
        "save unemployment_all_data, population2021_data, poverty_data, med_income_data into data folder"
    )
